...
/**
 * EN: Load a complete Dolibarr user to ensure getNomUrl displays the avatar.
 * FR: Charge un utilisateur Dolibarr complet pour garantir l'affichage de l'avatar via getNomUrl.
 *
 * @param DoliDB $db
 * @param int $userId
 * @return User|null
 */
function tw_load_user_with_photo($db, $userId)
{
	if (empty($userId)) {
		return null;
	}
	...
	if (!class_exists('User')) {
		// EN: Load the Dolibarr user class when not already available.
		// FR: Charge la classe utilisateur Dolibarr si elle n'est pas déjà disponible.
	...
	// EN: Normalise status and ensure photo is available for avatar rendering.
	// FR: Normalise le statut et s'assure que la photo est disponible pour l'avatar.
	$user->statut = (int) $user->statut;
