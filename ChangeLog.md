...
- Corrige les notifications de scellement automatique : le lien d'accès direct est construit depuis une URL publique absolue validée et enregistrée par entité, avec reprise de l'URL Multicompany puis de l'URL native Dolibarr uniquement si elles contiennent un hôte. L'activation initialise ce réglage sans écraser une valeur existante et la configuration refuse les racines incomplètes comme `https:/custom/...`. / Fixes automatic sealing notifications: the direct-access link is built from a validated absolute public URL stored per entity, with fallbacks to the Multicompany URL and then the native Dolibarr URL only when they contain a host. Module activation initializes this setting without overwriting an existing value, and setup rejects incomplete roots such as `https:/custom/...`.
- Supprime des courriels de scellement automatique les substitutions de signature utilisateur, de signature d'expéditeur et de pied de modèle contenant le nom de la société, sans modifier les autres notifications. / Removes the user signature, sender signature and company-name template footer substitutions from automatic sealing emails without changing other notifications.
- Fournit aux administrateurs un aperçu sans envoi du courriel résolu, basé sur la feuille d'identifiant `1` par défaut ou sur l'identifiant fourni dans l'URL. / Provides administrators with a non-sending preview of the resolved email, based on timesheet ID `1` by default or the identifier supplied in the URL.
...
- Renforce la compatibilité Multicompany, les contrôles de droits, les filtres d’entité, la protection CSRF et la conservation des réglages lors des désactivations et réactivations. / Strengthens Multicompany compatibility, permission checks, entity filtering, CSRF protection and settings preservation across module disable and re-enable cycles.
- Ajoute l’affichage natif des dernières feuilles sur la fiche bancaire utilisateur, améliore les liens `getNomUrl()`, les informations d’audit et harmonise les vues mobile et ordinateur avec les codes graphiques Dolibarr. / Adds native latest-timesheet display on the user bank card, improves `getNomUrl()` links and audit information, and aligns mobile and desktop views with Dolibarr design patterns.
- Établit le socle de compatibilité à Dolibarr 20+ et PHP 8.0+, ajoute l’onglet Compatibilité avec ses diagnostics et met en conformité la numérotation des permissions et les réglages administratifs. / Establishes the compatibility baseline at Dolibarr 20+ and PHP 8.0+, adds the Compatibility tab with diagnostics, and aligns permission numbering and administration settings.
