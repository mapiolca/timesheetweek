...
## 1.8.4 (29/06/2026)
- Ajoute l'affichage natif des dernières feuilles TimesheetWeek sur la fiche bancaire utilisateur lorsque le hook `printUserBankAfterExpenseReportArea` est disponible, avec droits lecture/propre/subordonnés/toutes et état dédié dans l'onglet Compatibilité. / Adds a native display of the latest TimesheetWeek rows on the user bank card when the `printUserBankAfterExpenseReportArea` hook is available, with own/subordinate/all read rights and a dedicated Compatibility tab status.
