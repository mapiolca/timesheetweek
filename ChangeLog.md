...
- Ajoute des notifications métier par étape avec modèles de courriel personnalisables (`timesheetweek_notification`) pour soumission, approbation, refus, retour en brouillon, scellement et descellement, déclenchées par leurs événements natifs dédiés. / Adds per-step business notifications with customizable email templates (`timesheetweek_notification`) for submission, approval, refusal, revert to draft, seal and unseal, triggered by their dedicated native events.
