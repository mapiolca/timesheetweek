...
## 1.8.4 (29/06/2026)
- Corrige l'affichage de l'événement TimesheetWeek dans la page native Notifications en synchronisant `c_action_trigger` depuis le hook `notifsupported` et en alignant le modèle routeur sur `timesheetweek@timesheetweek`. / Fixes TimesheetWeek event visibility in the native Notifications page by synchronizing `c_action_trigger` from the `notifsupported` hook and aligning the router template with `timesheetweek@timesheetweek`.
