...
- Garantit que le gabarit Dolibarr reçoit bien une chaîne de paramètres lors de lister les documents afin d'éviter les appels `preg_match` invalides. / Ensures the Dolibarr template receives a string parameter while listing documents to prevent invalid `preg_match` calls.
