...
- Corrige les notifications de scellement automatique : le lien d'accès direct utilise l'URL publique complète configurée pour les tâches CLI afin d'éviter les adresses incomplètes de type `https:/custom/...`, conserve `DOL_MAIN_URL_ROOT` pour les actions Web et n'ajoute plus la signature personnelle de l'utilisateur. Le test de scellement fournit également aux administrateurs un aperçu sans envoi du courriel résolu, basé sur la feuille d'identifiant `1` par défaut ou sur l'identifiant fourni dans l'URL. / Fixes automatic sealing notifications: the direct-access link uses the complete configured public URL for CLI jobs to prevent incomplete addresses such as `https:/custom/...`, retains `DOL_MAIN_URL_ROOT` for Web actions and no longer adds the user's personal signature. The sealing test also provides administrators with a non-sending preview of the resolved email, based on timesheet ID `1` by default or the identifier supplied in the URL.
