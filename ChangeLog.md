...
- Utilise le mode natif d'URL publique de `dol_buildpath()` uniquement en contexte CLI, afin que les courriels générés par la tâche planifiée reprennent l'URL complète définie dans `conf.php`, tout en conservant `DOL_MAIN_URL_ROOT` pour les notifications déclenchées depuis le Web. / Uses `dol_buildpath()`'s native public-URL mode only in CLI contexts so emails generated by the scheduled job use the complete URL defined in `conf.php`, while retaining `DOL_MAIN_URL_ROOT` for notifications triggered from the Web.
- Ajoute au test de scellement un aperçu administrateur, sans envoi, du modèle de courriel résolu avec le mode d'URL de la tâche planifiée, pour la feuille d'identifiant `1` par défaut ou pour l'identifiant fourni dans l'URL. / Adds an administrator-only, non-sending sealing test preview of the resolved email template using the scheduled-job URL mode, for timesheet ID `1` by default or the identifier supplied in the URL.
