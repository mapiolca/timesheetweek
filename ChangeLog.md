...
## 1.8.4 (29/06/2026)
- Ajoute l'affichage natif des dernières feuilles TimesheetWeek sur la fiche bancaire utilisateur via `formObjectOptions`, remonté sous les tableaux natifs de droite et limité par `MAIN_SIZE_SHORTLIST_LIMIT`, avec lien vers la liste préfiltrée sur l'utilisateur, droits lecture/propre/subordonnés/toutes et état dédié dans l'onglet Compatibilité. / Adds a native display of the latest TimesheetWeek rows on the user bank card through `formObjectOptions`, moved below the native right-column tables and limited by `MAIN_SIZE_SHORTLIST_LIMIT`, with a link to the list prefiltered on the user, own/subordinate/all read rights and a dedicated Compatibility tab status.
