...
/**
 *	\file       htdocs/core/modules/timesheetweek/doc/doc_generic_timesheetweek_odt.modules.php
 *	\ingroup    timesheetweek
 *	\brief      File of class to build ODT documents for timesheetweeks
 */
 ...
 */
class doc_generic_timesheetweek_odt extends ModelePDFTimesheetWeek
{
	/**
	 * Issuer
	 * @var Societe
	 ...
	 *
	 *  @param	TimesheetWeek	$object				Source object to generate document from
	...
	public function write_file($object, $outputlangs, $srctemplatepath = '', $hidedetails = 0, $hidedesc = 0, $hideref = 0)
	{
		...
		if ($conf->timesheetweek->dir_output) {
			// If $object is id instead of object
			if (!is_object($object)) {
				$id = $object;
				$object = new TimesheetWeek($this->db);
			...
			$dir = $conf->timesheetweek->multidir_output[isset($object->entity) ? $object->entity : 1];
			...
			if (file_exists($dir)) {
				...
				dol_mkdir($conf->timesheetweek->dir_temp);
				if (!is_writable($conf->timesheetweek->dir_temp)) {
					$this->error = $langs->transnoentities("ErrorFailedToWriteInTempDirectory", $conf->timesheetweek->dir_temp);
				...
				try {
					$odfHandler = new Odf(
						$srctemplatepath,
						array(
							'PATH_TO_TMP'	  => $conf->timesheetweek->dir_temp,
