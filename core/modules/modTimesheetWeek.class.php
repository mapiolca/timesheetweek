...
class modTimesheetWeek extends DolibarrModules
{
	...
	public function init($options = '')
	{
		...
		foreach ($nativeWorkflowRouterTemplates as $nativeWorkflowRouterTemplate) {
			...
			$sql[] = "UPDATE ".MAIN_DB_PREFIX."c_email_templates SET content = '".$this->db->escape($nativeWorkflowRouterTemplate['content'])."' WHERE module = 'timesheetweek' AND type_template = 'timesheetweek@timesheetweek' AND lang = '".$this->db->escape($nativeWorkflowRouterTemplate['lang'])."' AND label = '".$this->db->escape($nativeWorkflowRouterTemplate['label'])."' AND (content IS NULL OR content = '')";
		}
		...
		foreach ($workflowNotificationTemplates as $workflowNotificationTemplate) {
			$sql[] = "INSERT INTO ".MAIN_DB_PREFIX."c_email_templates (entity,module,type_template,lang,private,fk_user,datec,label,position,active,enabled,joinfiles,topic,content) SELECT 0,'timesheetweek','timesheetweek_notification','".$this->db->escape($workflowNotificationTemplate['lang'])."',0,NULL,NOW(),'".$this->db->escape($workflowNotificationTemplate['label'])."',".((int) $workflowNotificationTemplate['position']).",1,'isModEnabled(\\\"timesheetweek\\\")',0,'".$this->db->escape($workflowNotificationTemplate['topic'])."','".$this->db->escape($workflowNotificationTemplate['content'])."' WHERE NOT EXISTS (SELECT 1 FROM ".MAIN_DB_PREFIX."c_email_templates WHERE module = 'timesheetweek' AND type_template = 'timesheetweek_notification' AND lang = '".$this->db->escape($workflowNotificationTemplate['lang'])."' AND label = '".$this->db->escape($workflowNotificationTemplate['label'])."')";
		}
