...
if ($action === 'validate' && $id > 0) {
	...
	$object->status = TimesheetWeek::STATUS_VALIDATED; // statut interne conservé
	$object->date_validation = dol_now();
	if (empty($object->fk_user_valid)) $object->fk_user_valid = $user->id; // si non défini, on renseigne
	$res = $object->update($user);
	if ($res > 0) {
		// Message générique pour ne pas dépendre de la trad existante
...
JS;
	...
	if ($object->status == TimesheetWeek::STATUS_SUBMITTED) {
		...
		// Approuver / Refuser / Supprimer pour les validateurs
		if ($canValidator) {
			// Remplace libellé "Valider" => "Approuver"
			...
			// Supprimer si soumise et valideur
			echo dolGetButtonAction('', $langs->trans("Delete"), 'delete', $_SERVER["PHP_SELF"].'?id='.$object->id.'&action=delete&token='.$token);
		}
	}
...
(function($){
	$(function(){
		// Cible les badges de statut dans le banner
