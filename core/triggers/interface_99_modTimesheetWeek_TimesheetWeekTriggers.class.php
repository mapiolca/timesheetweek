...
class InterfaceTimesheetWeekTriggers extends DolibarrTriggers
{
	...
	protected function dispatchBusinessNotification($action, TimesheetWeek $timesheet, User $actionUser, $langs, $conf)
	{
		...
		$this->ensureTimesheetWeekSendTemplateExists($action, (int) $conf->entity);
	...
	/**
	 * Ensure the notification template selected for an action exists with type timesheetweek_send.
	 *
	 * @param string $action
	 * @param int    $entity
	 * @return void
	 */
	protected function ensureTimesheetWeekSendTemplateExists($action, $entity)
	{
		$templateLabel = trim((string) getDolGlobalString($action.'_TEMPLATE'));
		if ($templateLabel === '') {
			return;
		}
		...
		$sqlCheck .= " AND entity IN (0, ".((int) $entity).")";
		...
		$sqlSource .= " AND entity IN (0, ".((int) $entity).")";
