...
print '\tvar $limitSelect = $("select#limit");' . "\n";
print '\tif ($limitSelect.length && $.fn.select2) {' . "\n";
print '\t\t// EN: Mirror the select2 initialisation from stocktransfer_list.php to stay consistent with Dolibarr UX.' . "\n";
print '\t\t// FR: Reproduit l\'initialisation select2 de stocktransfer_list.php pour rester cohérent avec l\'UX Dolibarr.' . "\n";
print '\t\tvar normalizeString = function (value) {' . "\n";
print '\t\t\treturn (value || "").toLowerCase();' . "\n";
print '\t\t};' . "\n";
print '\t\t$limitSelect.select2({' . "\n";
print '\t\t\tdir: "ltr",' . "\n";
print '\t\t\twidth: "resolve",' . "\n";
...
print '\t\t\t\tif ($.trim(params.term) === "") {' . "\n";
print '\t\t\t\t\treturn data;' . "\n";
print '\t\t\t\t}' . "\n";
print '\t\t\t\tvar term = normalizeString(params.term);' . "\n";
print '\t\t\t\tvar text = normalizeString(data.text || "");' . "\n";
print '\t\t\t\tvar keywords = term.split(" ");' . "\n";
print '\t\t\t\tfor (var i = 0; i < keywords.length; i++) {' . "\n";
print '\t\t\t\t\tif (text.indexOf(keywords[i]) === -1) {' . "\n";
print '\t\t\t\t\t\treturn null;' . "\n";
print '\t\t\t\t\t}' . "\n";
print '\t\t\t\t}' . "\n";
print '\t\t\t\treturn data;' . "\n";
print '\t\t\t},' . "\n";
print '\t\t\ttheme: "default limit",' . "\n";
...
print '\t\t\t\tif (data.element) { $(container).addClass($(data.element).attr("class")); }' . "\n";
print '\t\t\t\tif (data.id == "-1" && $(data.element).attr("data-html") == undefined) {' . "\n";
print '\t\t\t\t\treturn "&nbsp;";' . "\n";
print '\t\t\t\t}' . "\n";
print '\t\t\t\tif ($(data.element).attr("data-html") != undefined) {' . "\n";
print '\t\t\t\t\tif (typeof htmlEntityDecodeJs === "function") {' . "\n";
print '\t\t\t\t\t\treturn htmlEntityDecodeJs($(data.element).attr("data-html"));' . "\n";
print '\t\t\t\t\t}' . "\n";
print '\t\t\t\t}' . "\n";
print '\t\t\t\treturn data.text;' . "\n";
print '\t\t\t},' . "\n";
print '\t\t\ttemplateSelection: function (selection) {' . "\n";
print '\t\t\t\tif (selection.id == "-1") {' . "\n";
print '\t\t\t\t\treturn "<span class=\"placeholder\">" + selection.text + "</span>";' . "\n";
print '\t\t\t\t}' . "\n";
print '\t\t\t\treturn selection.text;' . "\n";
print '\t\t\t},' . "\n";
print '\t\t\tescapeMarkup: function (markup) {' . "\n";
print '\t\t\t\treturn markup;' . "\n";
print '\t\t\t}' . "\n";
print '\t\t});' . "\n";
print '\t}' . "\n";
print '\t// EN: Trigger the Dolibarr refresh helper when the limit changes, just like stocktransfer_list.php.' . "\n";
print '\t// FR: Déclenche l\'assistant de rafraîchissement Dolibarr lors d\'un changement de limite, comme stocktransfer_list.php.' . "\n";
print '\t$(".selectlimit").off("change.timesheetweekLimit").on("change.timesheetweekLimit", function () {' . "\n";
print '\t\tvar $current = $(this);' . "\n";
print '\t\tvar selectedLimit = $current.val();' . "\n";
print '\t\tvar $targetForm = $current.parents("form:first");' . "\n";
print '\t\tif (!$targetForm.length) {' . "\n";
print '\t\t\t$targetForm = $("#searchFormList");' . "\n";
print '\t\t}' . "\n";
print '\t\tif ($targetForm.length) {' . "\n";
print '\t\t\tvar $limitHidden = $("#limit-hidden");' . "\n";
print '\t\t\tif (!$limitHidden.length) {' . "\n";
print '\t\t\t\t$limitHidden = $("<input>", {' . "\n";
print '\t\t\t\t\ttype: "hidden",' . "\n";
print '\t\t\t\t\tname: "limit",' . "\n";
print '\t\t\t\t\tid: "limit-hidden"' . "\n";
print '\t\t\t\t}).appendTo($targetForm);' . "\n";
print '\t\t\t}' . "\n";
print '\t\t\t$limitHidden.val(selectedLimit);' . "\n";
print '\t\t}' . "\n";
