...
if ($action === 'create') {
	...
	(function ($) {
		...
		function fmt(d){var dd=(\"\"+d.getUTCDate()).padStart(2,\"0\");var mm=(\"\"+(d.getUTCMonth()+1)).padStart(2,\"0\");var yy=d.getUTCFullYear();return dd+\"/\"+mm+\"/\"+yy;}
	...
	print dol_get_fiche_end(); // END HEADER ZONE
	...
	$object->fetchLines();
	$hoursByTaskByDay = array();
	$zoneByDay = array();
	$mealByDay = array();

	foreach ($object->lines as $line) {
		$dayN = (int) date('N', strtotime($line->day_date)); // 1..7
		$dn = $days[$dayN-1];
	...
	foreach ($days as $d) {
		...
		for ($z=0;$z<=5;$z++) print '<option value="'.$z.'"'.($z===$curZone?' selected':'').'>'.$z.'</option>';
		print '</select><br>';
	...
	(function($){
		...
		function updateTotals(){
			...
			$(\'.meal-total\').text($(\'input.mealbox:checked\').length);
			var weekly = '.((float)$contractedHours).';
			var ot = grand - weekly; if(ot<0) ot=0;
			$(\'.overtime-total\').text(formatHours(ot));
		}
		$(function(){ updateTotals(); $(document).on(\'input change\', \'input.hourinput, input.mealbox\', updateTotals); });
	})(jQuery);
	</script>';
}
